...
#pragma omp parallel for collapse(2) schedule(static) \
    ...
    for (unsigned int row = 0; row < result.size; row += m1.width) {
        for (unsigned int col = 0; col < m2.size; col += m2.width) {
            double sum = 0.0;
            auto rowBase = m1.container + row;
            auto colBase = m2.container + col;
            for (unsigned int dot = 0; dot < m2.width; dot++) {
                sum += rowBase[dot] * colBase[dot];
            }
            result[row / m1.width][col / m2.width] = sum;
        }
    }
    return result;
}
