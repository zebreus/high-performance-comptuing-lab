...
for LOCAL_RUN in $(seq 0 3); do
    ...
    srun --nodes=$NUM_NODES --ntasks=$NUM_TASKS --ntasks-per-node=$NUM_TASKS_PER_NODE --cpus-per-task=$NUM_THREADS --mem-per-cpu=$(min ${RAM_PER_CPU} 1000)M --threads-per-core=1 -- $ScratchDir/rust-sorting --work-directory ${ScratchDir} --algorithm "$ALGORITHM" $IN_FILE ${ScratchDir}/output${LOCAL_RUN} | grep -v "Start Singularity" >>$filename
    echo "" >>$filename
done
