...
run-benchmark() {
    ...
    # set -x
    sbatch --nice=200 --dependency=singleton --job-name=$JOBNAME --constraint="gold6248r" --nodes=${NUM_NODES} --cores-per-socket=24 --ntasks-per-node=1 --cpus-per-task=24 --threads-per-core=1 --error=${WORK_DIR}/%j.err -- ./run_benchmark.sh $NUM_NODES $OUT_FILE $EXECUTABLES_DIR
    # set +x
}
