...
Our project uses MPI via the rsmpi Rust bindings for distributed memory parallelism. Every MPI node will process a consectuive chunk of rows. For example if we have 2 nodes and 8 rows, node 0 will process rows 0-3 and node 1 will process rows 4-7. Our implementation is shown in <<core_with_mpi>>. Before each round every node sends its top row to the previous node and its bottom row to the next node. These way all nodes that are not at the top or bottom have the context to treat their top/bottom rows like core rows. The first and last nodes still process their rows like core rows.
...
// TODO: magick output_0.webp -repage 1040x1040 -coalesce null: \( output_1.webp -coalesce \) -geometry +0+260 -layers Composite null: \( output_2.webp -coalesce \) -geometry +0+520 -layers Composite null: \( output_3.webp -coalesce \) -geometry +0+780 -layers Composite    bag_append.gif
